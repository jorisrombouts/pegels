...
const tx = (over: Partial<Transaction>): Transaction => ({
  ...
  categoryId: "cat-groceries", predictedCategoryId: null, categoryConfidence: null, categoryLevel: null,
  categorySource: "model", needsReview: false, tagIds: [], kind: "expense", ...over,
});
  ...
  it("writes one multi-row upsert that restates every non-key column from excluded", async () => {
    ...
    expect(sent[0].sql).toBe(
      'insert into "transactions" ("id", "user_id", "date", "description", "amount", "account_id",' +
        ' "category_id", "predicted_category_id", "category_confidence", "category_level", "category_source",' +
        ' "needs_review", "excluded", "kind", "tag_ids", "splits", "notes") values' +
        ...
        ' "category_confidence" = excluded.category_confidence, "category_level" = excluded.category_level,' +
    ...
    // Every column the caller passed survives the round-trip, not just the patched ones. category_level
    // must be in the `set` clause too, or a recategorize writes the new label and keeps the old level.
    expect(sent[0].params.slice(17)).toEqual([
      "b", "u1", "2026-01-10", "ICA Maxi", "-100", "acc-lon", "cat-groceries", null, null, null, "model",
