...
describe("evaluate — confidence calibration", () => {
  ...
  it("measures how often a flagged row was actually wrong", () => {
    ...
    const m = evaluate(
      [gold(), gold(), gold()],
      [
        pred({ index: 0, confidence: 0.3, level: "low", categoryId: "cat-housing" }), // flagged, wrong  ✓
        pred({ index: 1, confidence: 0.3, level: "low" }), // flagged, right ✗
        pred({ index: 2, confidence: 0.9, level: "high" }), // not flagged
      ],
      maps,
    );
  ...
  it("reports zero review precision when nothing was flagged", () => {
    const m = evaluate([gold()], [pred({ confidence: 0.99, level: "high" })], maps);
